...
import {
  X, User, Mail, MapPin, Briefcase, Phone, CreditCard,
  DollarSign, Wallet, Banknote, Save, Trash2, Pencil
} from 'lucide-react';

export const UserAccount = ({ user, accounts, onClose, onUpdate, onDelete, deletingUserId }) => {
  ...
  const handleSubmit = async () => {
    const success = await onUpdate(user.uid, formData);
    if (success) {
      setEditMode(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-6">
      <motion.div
        ...
        className="bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl p-8 w-full max-w-4xl max-h-[90vh] overflow-y-auto relative border border-white/20 text-white"
      >
        <button
          onClick={onClose}
        ...
        <div className="grid md:grid-cols-2 gap-6 text-sm">
          <div className="space-y-2">
            ...
            <p>
              <Phone className="inline mr-2 text-[#9AF241]" />
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {accounts.map((acc) => (
                ...
                >
                  ...
                  <p><Banknote className="inline mr-2 text-[#9AF241]" /> <span className="font-semibold">Ingresos:</span> Q{acc.ingresosMensuales}</p>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-white/70 text-center">Este usuario no tiene cuentas registradas.</p>
          )}
        </div>
